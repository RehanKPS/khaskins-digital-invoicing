...
import { CalendarIcon, Plus, Trash2, Save, X, ChevronDown, AlertTriangle } from 'lucide-react';
...
const EditInvoicePage = () => {
  ...
  const { invoices, customers, products, categories, updateInvoice } = useData();
  ...
  useEffect(() => {
    if (invoice && !loaded) {
      ...
      setInvoiceDate(parseISO(invoice.invoiceDate));
      setItems(invoice.items.map(i => ({ ...i })));
      setCartage(invoice.cartage || 0);
      setScenarioId(invoice.scenarioId || 'SN001');
      setBuyerRegistrationType(invoice.buyerRegistrationType || '');
      const expanded: Record<number, boolean> = {};
      invoice.items.forEach((item, idx) => {
        if (item.subItems && item.subItems.length > 0) expanded[idx] = true;
      });
      setExpandedSubItems(expanded);
      setLoaded(true);
    }
  }, [invoice, loaded]);
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          <Button variant="outline" onClick={() => navigate(`/invoices/${invoice.id}`)}><X className="w-4 h-4 mr-2" /> Cancel</Button>
          <Button onClick={handleUpdate}><Save className="w-4 h-4 mr-2" /> Update Invoice</Button>
        </div>
      </div>

      <div className="flex items-start gap-3 p-4 bg-[hsl(var(--warning)/.1)] border border-[hsl(var(--warning)/.3)] rounded-lg">
        <AlertTriangle className="w-5 h-5 text-[hsl(var(--warning))] mt-0.5 flex-shrink-0" />
        <p className="text-sm">You are editing Invoice <strong>{invoice.invoiceNo}</strong>. Changes will reset FBR status to Not Sent.</p>
      </div>
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <Input value={invoice.invoiceNo} readOnly className="bg-muted" />
            </div>
            <div className="space-y-2">
